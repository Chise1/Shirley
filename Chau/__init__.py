...
from Shirley.config import settings

__all__ = ['register']


def register(app: FastAPI,  modules: Optional[List[str]] = None, generate_schemas: bool = True,
    ...
    init(settings.DB_URL, all_modules)
    register_tortoise(  # 这里是启动app的，之后会考虑和使用uvicorn启动的性能差别
        app,
        db_url=settings.DB_URL,  # 数据库信息
    ...
    app.include_router(router, prefix=settings.LOGIN_URL, tags=['admin'])